...
import { createConfiguredLlmJsonClient } from "../services/LlmJsonClientFactory";
import type {
	BuildingFact,
...
function deriveDocumentMetadataFromFact(fact: BuildingFact): {
	houseId?: string;
	apartmentId?: string;
	unitLabel?: string;
} {
...
export async function runBaselineDryRun({
	...
	if (!gatekeeper || !extractor) {
		...
		if (!gatekeeper) {
			const llmClient = createConfiguredLlmJsonClient("gatekeeper", runtimeEnv);
			resolvedGatekeeper = new Gatekeeper(llmClient, {
				strictErrors: strictAiErrors,
			});
		}
		if (!extractor) {
			const llmClient = createConfiguredLlmJsonClient("extractor", runtimeEnv);
	...
	if (
		...
		process.env.GEMINI_MODEL_EMBEDDING
	) {
		resolvedEmbeddingClient = new GeminiEmbeddingService();
	}
	const resolvedSemanticIndexService =
		semanticIndexService ??
		new SemanticIndexService(db, resolvedEmbeddingClient);
	...
	let remainingCaseBudget =
		maxCasesPerRun !== undefined && maxCasesPerRun >= 0
			? maxCasesPerRun
			: undefined;
	...
	for (const ingestion of ingestions) {
		...
		const persistedInThisFile: Array<{
			id: string;
			key: string;
			value: string;
		}> = [];
		...
		for (const fact of factsForFile) {
			...
			if (
				resolvedScope.scopeType === "house" ||
				resolvedScope.scopeType === "apartment"
			) {
				if (!resolvedScope.houseId) {
					throw new Error(
						"Resolved house/apartment scope is missing required houseId",
					);
				}
			...
			if (resolvedScope.scopeType === "apartment") {
				if (!resolvedScope.apartmentId) {
					throw new Error(
						"Resolved apartment scope is missing required apartmentId",
					);
				}
			...
			await resolvedSemanticIndexService.refreshFactEmbeddingById(
				enrichedFact.id,
			);
		...
		if (caseExtractor && !isCoreIngestion) {
			...
			const additionallyLinkedCaseIds =
				await lifecycle.linkFactsToOpenCasesHeuristic({
					propertyId,
	...
	const goldFactsPersisted = persistedFacts.filter(
		(fact) => fact.isGoldStandard,
	).length;
