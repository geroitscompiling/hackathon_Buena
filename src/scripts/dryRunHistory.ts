...
async function preloadExistingGoldFacts(propertyId: string) {
	const existingFacts = await db.query.facts.findMany({
		where: (factsTable, { and, eq }) =>
			and(
				eq(factsTable.propertyId, propertyId),
				eq(factsTable.isGoldStandard, true),
			),
		with: {
			houseLinks: true,
			apartmentLinks: true,
...
async function main() {
	try {
		...
		if (mode !== "mock" && mode !== "live") {
			throw new Error(
				`Unsupported HISTORY_MODE: ${mode}. Use "mock" or "live".`,
			);
		}
		const dayFilter =
			process.env.HISTORY_DAY?.trim() || process.env.DAY?.trim() || undefined;
		...
		const replay = await runPropertyHistoryReplay({
			...
			runDay: async ({ dayLabel, datasetRootPath, noisyInputFiles }) => {
				const preloadedExistingFacts =
					await preloadExistingGoldFacts(propertyId);
